...
public interface PostRepo extends JpaRepository<PostEntity, String> {
    ...
    @Query(value = "SELECT count(p) FROM PostEntity p WHERE p.isDelete = 0 AND p.createDate >= :startDate AND p.createDate <= :endDate")
    Optional<Long> countByDate(LocalDateTime startDate, LocalDateTime endDate);
    @Query(value = "SELECT count(p) FROM PostEntity p WHERE p.isDelete = 0")
